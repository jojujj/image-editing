	...
	zoomfunct:function($, $img, s){
		...
		var $zoomimages=$('<img src="'+magnifyicons[0]+'" /><img src="'+magnifyicons[1]+'" />')
			.css({width:magnifyicons[2], height:magnifyicons[3], cursor:'pointer', zIndex:1000, position:'absolute',
						top:s.wrappersize[1]-magnifyicons[3]-1, left:s.wrappersize[0]-magnifyicons[2]-3, opacity:1
			})
			.attr("title", "Zoom Out")
		...
		$zoomimages.click(function(e){ //assign click behavior to zoom images
			...
			if (zoomtype=="in" && s.curzoom==ddimagepanner.maxzoom || zoomtype=="out" && s.curzoom==0.2) //exit if user at either ends of magnify levels
				return
			var basepos=[s.pos[0]/curzoom, s.pos[1]/curzoom]
			var newzoom=(zoomtype=="out")? Math.max(0.4, curzoom-0.2) : Math.min(ddimagepanner.maxzoom, curzoom+0.2) //get new zoom level
			newzoom=Math.round(newzoom*10)/10
			$zoomimages.css("opacity", 1)
			if (newzoom==0.4) //if zoom level is 1x, dim "zoom out" image
...
function callpanner(){ //By default look for DIVs with class="pancontainer"
	var $pancontainer=jQuery('div.pancontainer')
	$pancontainer.each(function(){
		var $this=jQuery(this).css({position:'relative', overflow:'hidden', cursor:'move'})
